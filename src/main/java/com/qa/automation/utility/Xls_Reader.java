...
import org.apache.poi.xssf.usermodel.XSSFWorkbook;
	...
	public class Xls_Reader {
		...
		// get number of executable rows present in the sheet
		public int getExecutableRowCount(String SheetName, String testCaseName) {
			int ExecutableRowCOunt = 0;
			try {
				XSSFSheet Sheetname = workbook.getSheet(SheetName);
				for (int i = 1; i <=Sheetname.getLastRowNum(); i++) {
					
					String Col1CellData=getCellData(SheetName, 0, i);
					String Col2CellData=getCellData(SheetName, 1, i);
					if (getCellData(SheetName, 0, i).equalsIgnoreCase("Yes") && getCellData(SheetName, 1, i).equalsIgnoreCase(testCaseName)) {
						ExecutableRowCOunt++;
					}
				}
